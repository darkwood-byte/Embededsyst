...
void rotary_encoder_callback(uint gpio, uint32_t events) {
    ...
    if (gpio == ROMOTE_GPIO && (events & GPIO_IRQ_EDGE_RISE )){
        if ((REMOTE_MS * 1000) < time_us_64() - last_remote_triger){
        ...
        if (remote_buffer_index > sizeof(remotebuffer) / sizeof(int) - 1){
            ...
            */
            ...
            case 3515:
                ...
                printf("4\n");
                delta--;
                if(mode == 0)max_delta = NUM_LEDS - 1;
...
void led_run(void){
    for (int i = 0; i < NUM_LEDS; single_led(i), sleep_ms(15 + delta), i++);
    return;
}
